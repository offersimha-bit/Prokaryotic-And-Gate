...
class ProkaryoticSwitchGenerator:
    ...
    def __init__(self, translated_gene_sequence: str, leader_sequence: str = "GGG",
                 ...
                 nucleotides_count_to_trim_gene_after: int = 30, trigger_length: int = 30):
        ...
        self.trigger_length = trigger_length #a_domain_size + b_domain_size_pre_bulge + b_domain_size_post_bulge + len(START_CODON)
    ...
    def get_switch(self, trigger_sequence: str, healthy_sequence: str = None,
        ...
        if is_watson_crick:
            ...
            b_domain_pre_bulge = Domain(
                f"{trigger_reverse_complement[self.a_domain_size:self.a_domain_size + self.b_domain_size_pre_bulge]}{'N' * max(0, self.b_domain_size_pre_bulge - len(trigger_reverse_complement[self.a_domain_size:]))}",
                name="b_domain_pre_bulge", material="rna")
            b_domain_bulge = Domain(
                f"{trigger_reverse_complement[self.a_domain_size + self.b_domain_size_pre_bulge:self.a_domain_size + self.b_domain_size_pre_bulge + len(START_CODON)]}{'N' * max(0, len(START_CODON) - len(trigger_reverse_complement[self.a_domain_size + self.b_domain_size_pre_bulge:]))}",
                name="b_domain_bulge", material="rna")
            b_domain_post_bulge = Domain(
                f"{trigger_reverse_complement[self.a_domain_size + self.b_domain_size_pre_bulge + len(START_CODON):self.a_domain_size + self.b_domain_size_pre_bulge + len(START_CODON) + self.b_domain_size_post_bulge]}{'N' * max(0, self.b_domain_size_post_bulge - len(trigger_reverse_complement[self.a_domain_size + self.b_domain_size_pre_bulge + len(START_CODON):]))}",
